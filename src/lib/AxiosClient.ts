...
import { getCookie, eraseCookie, showToast } from '@/src/lib/Utils'
...
axiosClient.interceptors.request.use(
  (request: any) => {
    ...
    const userInfo: any = getCookie(AXIOS_CONFIG.TOKEN) || null

    const partUserInfo: any = userInfo ? JSON.parse(userInfo as any) : {}

    if (partUserInfo) request.headers.Authorization = `Bearer ${partUserInfo?.data?.accessToken}`
